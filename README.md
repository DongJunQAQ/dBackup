...
```
[root@localhost ~]# dBackup resource --type ecs
┌──────────────────────────────────────────────────────┐
| 服务器ID                              | 服务器名称      |
| cee2e312-df89-446b-8004-a9fe00d19793 | ecs-d872-eaf2 |
└──────────────────────────────────────────────────────┘
...
```
[root@localhost ~]# dBackup resource --type evs
┌───────────────────────────────────────────────────────────────────────────────────────┐
| 云硬盘ID                              | 云硬盘名称    | 容量(GB) | 是否为启动盘| 挂载服务器ID| 
| 324f7e53-4e10-4162-8d2a-7ce8de011c62 | volume-cb18  | 10      | false     | 未进行挂载  | 
| 2fff7ffc-fbaf-4c5a-904a-01c9e2cae180 | ecs-d872-eaf2| 10      | true      | cee2e312. | 
└───────────────────────────────────────────────────────────────────────────────────────┘
 SUCCESS  共查询到 2 个EVS云硬盘
```
...
SUCCESS  存储库f1840521-6103-4bf3-9198-df019ee51d22正在执行备份请稍后...
```
